...
def test_critique_rejects_unknown_rewrite_target() -> None:
    with pytest.raises(ValidationError):
        CritiqueArtifact(
            dimension_scores={
                "hook_strength": DimensionCritique(
                    score=7, reason="ok", evidence="line 1", improvement_advice="sharpen"
                ),
                "character_consistency": DimensionCritique(
                    score=7, reason="ok", evidence="line 2", improvement_advice="keep"
                ),
                "conflict_intensity": DimensionCritique(
                    score=6, reason="ok", evidence="line 3", improvement_advice="raise stakes"
                ),
                "pacing": DimensionCritique(
                    score=6, reason="ok", evidence="line 4", improvement_advice="trim"
                ),
                "short_drama_feel": DimensionCritique(
                    score=7, reason="ok", evidence="line 5", improvement_advice="push twist"
                ),
                "ending_payoff": DimensionCritique(
                    score=5, reason="ok", evidence="line 6", improvement_advice="pay off"
                ),
                "language_fluency": DimensionCritique(
                    score=8, reason="ok", evidence="line 7", improvement_advice="keep"
                ),
            },
            overall_score=6.6,
...
def test_settings_defaults_use_repo_relative_paths(
    ...
    assert settings.enable_originality_plan is False
    assert settings.originality_planner_model_name == "originality-planner"
    assert settings.originality_planner_max_retries == 1
